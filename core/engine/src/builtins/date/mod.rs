...
impl Date {
    ...
    pub(crate) fn to_iso_string(
        ...
        let year = if year.is_positive() && year >= 10000 {
            js_string!(
                "+",
                JsStringSlice::from(pad_six(year.unsigned_abs()).as_slice())
            )
        ...
        let binding = pad_two(month_from_time(tv) + 1);
        let month = JsStringSlice::from(binding.as_slice());
        ...
        let binding = pad_three(ms_from_time(tv));
        let millisecond = JsStringSlice::from(binding.as_slice());
