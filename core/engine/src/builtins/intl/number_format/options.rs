...
use crate::{
    ...
    js_string,
    string::JsStringSlice,
    Context, JsNativeError, JsObject, JsResult, JsString,
};
...
impl Unit {
    ...
    pub(crate) fn to_js_string(&self) -> JsString {
        if self.denominator.is_empty() {
            ...
            let numerator = JsStringSlice::from(self.numerator);
            let denominator = JsStringSlice::from(self.denominator);
            js_string!(numerator, js_str!("-per-"), denominator)
        }
    }
}
